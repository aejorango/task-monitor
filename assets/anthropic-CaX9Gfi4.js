...
`)}\n\nWrite the summary.`,maxTokens:1200,meta:{kind:`weekly-summary`}})).trim()}async function f({tasks:e,projects:t,today:n,count:r=3}){let a={};t.forEach(e=>{a[e.id]=e});let o=e.filter(e=>e.status!==`done`).slice(0,60),s=o.map(e=>{let t=a[e.projectId];return`- id:${e.id} | ${e.title} [${e.status}/${e.priority||`medium`}]${t?` · ${t.name}`:``}${e.plan?.endDate?` · due ${e.plan.endDate}`:``}${e.dependsOn?.length?` · blocked by ${e.dependsOn.length} task(s)`:``}`}),c=await i({system:`You help an operator who feels stuck decide what to do next. From their open tasks, choose the ${r} MOST important to tackle right now. Weigh: due dates (sooner = higher), priority, dependencies (a task blocked by incomplete work ranks lower; one that unblocks others ranks higher), and momentum.

Respond ONLY with a JSON array of exactly ${r} objects, highest priority first. Each object:
...
No prose, no markdown, no code fences. The response must start with [ and end with ].`,user:`Today is ${n}.
Open tasks (${o.length}):
${s.join(`
`)||`(none yet — propose sensible first actions)`}

Return the top ${r} as a JSON array.`,maxTokens:700,meta:{kind:`top-tasks`}});if(!Array.isArray(c))throw Error(`Model did not return a JSON array.`);let l=new Set(o.map(e=>e.id));return c.slice(0,r).map((e,t)=>({id:`sug-${t}`,taskId:e.taskId&&l.has(e.taskId)?e.taskId:null,title:String(e.title||``).slice(0,160).trim()||`Suggestion ${t+1}`,reason:String(e.reason||``).slice(0,280).trim()}))}async function p({tasks:e,projects:t,today:r}){let i={};return t.forEach(e=>{i[e.id]=e}),(await n({system:`You help an operator prioritize. Given a list of open tasks, recommend what to tackle TODAY in order. Consider: due dates, priority, dependencies (a task blocked by an incomplete dep should rank lower), and whether it unblocks others.
...
`)}\n\nWhat should I tackle today?`,maxTokens:800,meta:{kind:`next-task`}})).trim()}async function m({activities:e,audience:t=`a teammate`}){let r=e.map(e=>`- ${e.date} · ${e.taskTitle||``}${e.comment?`: ${e.comment}`:``}`);return(await n({system:`You draft brief, factual status updates. The audience is ${t}.
...
`)}\n\nWrite the status update.`,maxTokens:800,meta:{kind:`status-update`}})).trim()}export{u as a,p as c,c as i,f as l,a as n,s as o,m as r,o as s,i as t,d as u};